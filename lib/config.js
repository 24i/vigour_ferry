...
config.items = {
  ...
  'vigour.packer.robots': {
    ...
    env: 'PACKER_ROBOTS',
    cli: '--no-robots',
    desc: 'Serves `ferry/robots.txt` on `GET /robots.txt` by default. Use --no-robots to serve the `robots.txt` from your app instead'
  },
  'vigour.packer.geo': {
    def: true,
    env: 'PACKER_GEO',
    cli: '--no-geo',
    desc: 'Serves `ferry/geo.json` on `GET /geo` by default. Use --no-geo to remove this feature'
  },
  ...
  'vigour.packer.cleanup': {
    ...
    env: 'PACKER_CLEANUP',
    cli: '-x, --cleanup',
    desc: 'Removes all downloaded and created assets, state.json, and history.json transformHistory.json is conserved'
  },
  ...
  'vigour.packer.release': {
    ...
    env: 'PACKER_RELEASE',
    cli: '-r, --release',
    desc: 'Commits assets declared in app\'s package.json to sepcified branch of release repo (<this_repo>-packer-release), creating it if needed'
  },
