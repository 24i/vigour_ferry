...
exports.acceptHookshots = function () {
  gitListener = github.listen(cfg.git.port.val)
  log.info('Listening for hookshots on port', cfg.git.port.val)
  return gitListener
}
  ...
  function finish () {
    if (r && h) {
      ...
      try {
        ...
        if (branch === cfg.git.branch.val || branch === '_local') {
          sha = (branch === '_local')
            ? '_local'
            : h
          offerSha(sha)
