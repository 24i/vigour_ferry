...
function release () {
  var self = this
  return getReleaseRepo(self.config)
    .then(function () {
      return createSSHConfigFile()
    })
    .then(function () {
      return git.checkoutRelease(self.config)
    })
    .then(function () {
      return git.pullRelease(self.config)
    })
    .then(function () {
      return syncAssets(self.config)
    })
    .then(function () {
      return git.commitRelease(self.config)
    })
...
function getReleaseRepo (config) {
  ...
  .then(function () {
    return new Promise(function (resolve, reject) {
      fs.exists(config.releaseRepo.absPath.val, function (exists) {
        var returns
        if (exists) {
          returns = true
        } else {
...
function getGitBranch (config) {
  return readFile(path.join(process.cwd(), '.git', 'HEAD'), 'utf8')
    .then(function (data) {
      config.git.branch.val = (data.slice(data.lastIndexOf('/') + 1)).trim()
    })
}
    ...
    helpers.sh('rm -rf *'
      ...
      , function (error, stdout, stderr) {
        if (error) {
          ...
          resolve(fs.expandStars(config.ferry.assets.plain(), process.cwd())
            .then(function (val) {
              return flatten(val, '/')
            })
            .then(function (newAssets) {
              ...
              for (key in newAssets) {
                ...
                arr.push(copyNonEmpty(filePath, path.join(config.releaseRepo.absPath.val, key)))
              }
