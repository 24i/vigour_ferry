...
function Version (sha, config) {
  ...
  self.root = path.join(self.config.assetRoot.val, self.config.shaDir.val, self.sha)
  self.packagePath = path.join(self.root
    , 'package.json')
  self.manifestPath = path.join(self.root
    , self.config.buildDir.val
    , 'manifest.json')
  ...
  self.getPkg = helpers.getter(function () {
    return readJSON(self.packagePath)
      .then(function (parsed) {
        parsed.sha = self.sha
        parsed.repository.branch = self.config.git.branch.val
        if (self.config.git.branch.val === 'dev') {
...
Version.prototype.getAsset = function (file, params, shaRequest) {
  ...
  return self.getPkg()
    ...
    .then(function (pkg) {
      var allTransforms = pkg.vigour.ferry.transforms || {}
      var transforms
      if (file.indexOf('/') === 0) {
        file = file.slice(1)
      }
      if (file === '') {
        file = pkg.vigour.ferry.web
      }
      ...
      if (transforms) {
        params.langs = pkg.vigour.languages
...
Version.prototype.getTransformed = function (file, transforms, params, shaRequest) {
  ...
  // console.log('getting transformed version of', originalPath)
  transformedPath = originalPath +
    '_' +
    id +
    path.extname(file)
  return stat(transformedPath)
    .catch(function (reason) {
      if (reason.code === 'ENOENT') {
        return false
      } else {
        state.log("Can't call stat")(reason)
      }
    })
    .then(function (stats) {
      var p
      var stillValid = transforms.indexOf('meta') !== -1
        ? stats && Date.parse(stats.mtime) + self.config.jsonldmaxage.val > Date.now()
        : true
      if (stats && stillValid) {
    ...
    .then(function (history) {
      self.getPkg()
        ...
        .then(function (pkg) {
          ...
          var arr = []
          for (file in history) {
            if (pkg.vigour.ferry.transforms[file]) {
...
Version.prototype.makeAppCacheManifest = function (pkg) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      acm = 'CACHE MANIFEST\n'
      acm += '#' + pkg.version + '\n'
      acm += pkg.vigour.ferry.web + '\n'
      self.expandStars(pkg.vigour.ferry.assets
        , self.root
        , function (err, extended) {
          if (err) {
            err.message += ': Error expanding stars'
            err.path = pkg.vigour.ferry.assets
            reject(err)
            return
          }
          // Next
      ...
      console.warn('Do you have `vigour.ferry.web` and `vigour.ferry.assets` in your package.json?')
    }
...
Version.prototype.makeManifest = function (pkg) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var assets
    manifest.main = pkg.vigour.ferry.main
...
Version.prototype.archive = function () {
  ...
  return shaHistory.get()
    ...
    .then(function (history) {
      ...
      while (history.length >= self.config.maxHistory.val) {
        mustErase = true
        history.shift()
      }
      if (mustErase) {
...
Version.prototype.removeShas = function (history) {
  var self = this
  return readdir(path.join(self.config.assetRoot.val
      , self.config.shaDir.val))
    ...
    .then(function (files) {
      ...
      return Promise.all(
        toErase.map(function (item) {
          return remove(path.join(self.config.assetRoot.val
              , self.config.shaDir.val
              , item))
...
Version.prototype.download = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (self.sha === '_local') {
      ...
      options = {
        hostname: self.config.git.api.hostname.val,
        path: '/' + path.join('repos',
          self.config.git.owner.val,
          self.config.releaseRepo.name.val,
          'git',
          'commits',
          self.sha),
        headers: self.config.git.api.headers.plain(),
        method: 'HEAD'
      }
      var req = https.request(options
        , function (res) {
          ...
          if (res.statusCode !== 200) {
            error = new Error("GitHub doesn't return 200 for this sha")
            error.sha = self.sha
...
Version.prototype.getLocal = function () {
  ...
  log.info(helpers.hNow() + ' Copying ', self.config.src.val)
  return helpers.sh('cp -fR ' +
        (path.isAbsolute(self.config.src.val)
          ? self.config.src.val
          : path.join(process.cwd(), self.config.src.val)) +
        ' ' +
        self.sha
      , { cwd: path.join(self.config.assetRoot.val, self.config.shaDir.val) })
    .then(function () {
...
Version.prototype.clone = function () {
  var self = this
  log.info('heapUsed: ', process.memoryUsage().heapUsed)
  log.info(helpers.hNow() + ' Cloning ', self.config.releaseRepo.name.val)
  return helpers.sh('git clone --depth=1 -b ' +
        self.config.git.branch.val +
        ' ' +
        self.config.git.url.val +
        ':' +
        self.config.git.owner.val +
        '/' +
        self.config.releaseRepo.name.val +
