...
function getMetaFile (_url, filename, region) {
  ...
  var url = `${_url}/data/` + template({ region: region })
...
Transformer.prototype.langs = function (data, params, pkg, version) {
  var base = params.path.replace(/(\?|&)lang=[a-zA-Z]{2}/, '')
  var insert = '\n'
  ...
  for (var key in params.langs) {
    ...
    insert += `<link rel="alternate" hreflang="${lang}" href="http://${params.host}${base}${langPart}${lang}" />\n`
  }

  insert += '</head>'
  return Promise.resolve(data.replace('</head>', insert))
}
Transformer.prototype.langs.params = ['originalUrl', 'langs', 'host']
